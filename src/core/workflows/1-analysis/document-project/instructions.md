# Document Project — Document Output

<critical>You MUST have already loaded and processed: the workflow.yaml for this workflow</critical>

## Overview

Scans a brownfield project and produces comprehensive documentation on the platform. The user selects which aspects to document (API, architecture, setup, or all), and the workflow generates structured documentation published as documents.
...
<step n="4" goal="Publish documentation to document">
<action>Invoke the `write-document` task with:</action>

```
title: "[{team_name}] Documentation: {project_name}"
...
**Next Steps:**
1. Review the documentation on the platform for accuracy
