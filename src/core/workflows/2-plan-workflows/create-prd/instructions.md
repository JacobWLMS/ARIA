...
Search the platform for an existing PRD:
- Invoke the `read-context` task from `{project-root}/_aria/core/tasks/read-context.md` with `context_type: "document_artefact"` and `query: "PRD"` to search for documents with "PRD" or "Product Requirements" in the title
- If found, offer the user a choice:

{if autonomy_level == "interactive"}
  "I found an existing PRD on the platform: {prd_title}. Would you like to:
...
If resuming: load the existing document content via the `read-context` task, present a progress summary, and skip to the first incomplete section based on what content exists. Treat any section with substantive content as complete.

### 1.2 Load Context from the platform

Invoke the `read-context` task from `{project-root}/_aria/core/tasks/read-context.md` with `context_type: "project_overview"` to fetch existing project context from documents (product brief, research reports, brainstorming notes).

**Context Discovery — search for these artefacts on the platform:**
- Product Brief: invoke `read-context` with `context_type: "document"` and `scope_key: "product_brief"`
- Research Reports: invoke `read-context` with `context_type: "document_artefact"` and `query: "Research"`
- Brainstorming: invoke `read-context` with `context_type: "document_artefact"` and `query: "Brainstorming"`
- Project Context: invoke `read-context` with `context_type: "document_artefact"` and `query: "Project Context"`

Load ALL discovered documents completely via the `read-context` task. Confirm findings with the user before proceeding:
...
**Context Loaded from the platform:**
...
- Context loaded from documents (not local files)
...
- Looking for local files instead of documents
...
- IF C: Hold the approved content in working memory for final document write. Proceed to step 5.
...
- Writing to document prematurely (all content accumulates until step 12)
...
<step n="12" goal="Polish complete PRD, write to document, link to Projects, and hand off">

**Progress: Step 12 of 12** — Final Step

### Step Goal

Polish the complete PRD for flow and coherence, write it to a document, link to epics (epics), and post a handoff to downstream agents.
...
{if autonomy_level == "interactive"}
  "Review the PRD summary above. [C] Continue to publish to the platform, or provide feedback to revise."
...
### 12.5 Write PRD to document

Compile the complete PRD using the template at `{template}`.
Generate the document in {document_output_language}.

Invoke the `write-document` task from `{project-root}/_aria/core/tasks/write-document.md` with:
...
Update `{key_map_file}` with the new document ID under `documents.prd`.

### 12.6 Link PRD to Existing Projects in the platform

Search for existing epics (epics):
Call `list_projects` to find projects for the team.

For each Project found, invoke the platform's project update to update its description with the PRD link:
...
Invoke the `post-handoff` task from `{project-root}/_aria/core/tasks/post-handoff.md` with:

```
handoff_to: "Architect,SM"
handoff_type: "prd_complete"
summary: "PRD created and published to document. Ready for architecture design and epic planning."
...
**Next Steps:**
1. Review the PRD on the platform
...
- Complete PRD written to document (not local files)
...
### Failure Modes
- Writing PRD to local files instead of document
