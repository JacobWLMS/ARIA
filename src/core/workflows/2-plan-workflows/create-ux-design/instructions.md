# Create UX Design — Document Output

<critical>You MUST have already loaded and processed: the workflow.yaml for this workflow</critical>

## Overview

Creates a comprehensive UX design specification through a 14-step collaborative visual exploration workflow. All output is written to a document exclusively. After creation, links the UX Design to epics and posts a handoff.
...
<step n="1" goal="Initialize — load context from the platform and detect continuation">

<action>Check for existing UX design on the platform:</action>
<action>Invoke the `read-context` task from `{project-root}/_aria/core/tasks/read-context.md` with `context_type: "document_artefact"` and `query: "UX Design"` to search for documents with "UX Design" or "UX Specification" in the title</action>

If found:
- Read document content via the `read-context` task
...
<action>Invoke `read-context` from `{project-root}/_aria/core/tasks/read-context.md` with `context_type: "project_overview"`</action>
<action>Invoke `read-context` with `context_type: "document"` and `scope_key: "prd"` — **required prerequisite**</action>
<action>Invoke `read-context` with `context_type: "document"` and `scope_key: "product_brief"` (if exists)</action>
<action>Invoke `read-context` with `context_type: "document"` and `scope_key: "architecture"` (if exists)</action>
...
<step n="14" goal="Compile UX design, write to document, link to Projects, and hand off">
...
<action>Update `{key_map_file}` with the document ID under `documents.ux_design`</action>

<action>Link UX design to existing epics:</action>
<action>Call `list_projects` to find projects for the team</action>
<action>For each Project, invoke the platform's project update to update its description with the UX Design link:</action>
...
summary: "UX design specification created and published to document. Ready for architecture alignment and story creation."
...
**Next Steps:**
1. Review UX design on the platform
2. Architect can align architecture with UX requirements
3. SM can incorporate UX specs into story acceptance criteria"

<action>Invoke help task at `{project-root}/_aria/core/tasks/help.md`</action>

**Success:** Complete UX written to document; key map updated; Projects linked; handoff posted.
