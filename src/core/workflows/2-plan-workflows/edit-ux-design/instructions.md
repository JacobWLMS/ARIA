# Edit UX Design — Document Update

<critical>You MUST have already loaded and processed: the workflow.yaml for this workflow</critical>

<workflow>

<step n="1" goal="Load the UX design and any feedback from the platform">
...
### 1.1 Locate the UX Design on the platform

Invoke the `read-context` task from `{project-root}/_aria/core/tasks/read-context.md` with `context_type: "document"` and `scope_key: "ux_design"` to locate the UX design document.

If the UX design is not found via key map, search the platform directly:
- Invoke the `read-context` task with `context_type: "document_artefact"` and `query: "UX Design"` to search for documents with "UX Design" or "UX Specification" in the title
...
- "I cannot find an existing UX design on the platform. Please check or create one first with Create UX Design."
- EXIT workflow.

### 1.2 Load Full UX Design Content

Load the full UX design content via the `read-context` task with the selected document ID. Store the `document_id`, `title`, and content for later update.
...
Invoke `read-context` with `context_type: "document"` and `scope_key: "prd"` to load the PRD (if exists) for cross-reference during edits.
...
"**Edit Mode: Updating existing UX Design on the platform.**
...
### Success Criteria
- UX design located and fully loaded from the platform
- Comments retrieved
...
<step n="4" goal="Update the document with the edited UX design">
...
**Next Steps:**
1. Review the updated UX design on the platform
