# Check Implementation Readiness — Platform Assessment

<critical>You MUST have already loaded and processed: the workflow.yaml for this workflow</critical>

<workflow>

<step n="1" goal="Discover and load all project artefacts from the platform">
...
### 1.1 Load Artefacts from documents

Invoke the `read-context` task from `{project-root}/_aria/core/tasks/read-context.md` with `context_type: "project_overview"` to discover all available project artefacts.

Load each artefact from documents:
- Invoke `read-context` with `context_type: "document_artefact"` and `scope_id: "prd"` to load the PRD
- Invoke `read-context` with `context_type: "document_artefact"` and `scope_id: "architecture"` to load the Architecture
- Invoke `read-context` with `context_type: "document_artefact"` and `scope_id: "ux_design"` to load the UX design (if exists)
- Invoke `read-context` with `context_type: "document_artefact"` and `scope_id: "product_brief"` to load the product brief

### 1.2 Load Projects (Epics) and Issues (Stories) from the platform

Search for all Projects (Epics):
- Call `list_projects` with `team: "{team_name}"` to find all ARIA-managed projects

For each Project found, load its issues:
- Invoke the `read-context` task from `{project-root}/_aria/core/tasks/read-context.md` with `context_type: "work_items"`, `project: "{project_id}"` to get all issues under this project
...
Invoke the `post-comment` task from `{project-root}/_aria/core/tasks/post-comment.md` with:
...
This creates targeted feedback visible in the platform UI for the PM to address.
...
Invoke the `post-comment` task with:
...
<step n="6" goal="Compile final assessment, publish to the platform, and hand off">

**Progress: Step 6 of 6** — Final Step

### Step Goal

Compile all findings from steps 2-5 into a comprehensive readiness report, determine the overall verdict, publish to document, post Project-specific comments, and hand off to the SM agent.
...
### 6.4 Publish to document

Invoke the `write-document` task from `{project-root}/_aria/core/tasks/write-document.md` with:

```
title: "[{team_name}] Implementation Readiness Report: {project_name} ({date})"
body_content: "{compiled_readiness_report}"
key_map_id: "readiness_report"
```

Update `{key_map_file}` with the new document ID under `documents.readiness_report`.

### 6.5 Post Project-Specific Comments in the platform

For each Project (Epic), post a summary comment on a representative issue with findings specific to that Project:

<action>Invoke the `read-context` task with `context_type: "work_items"`, `project: "{project_id}"`, `limit: 1` to find a representative issue</action>
...
body: |
  ...
  **Date:** {date}
  **Report:** See document "{readiness_report_title}"
...
summary: "Implementation readiness check completed. Verdict: {verdict}. Report published to document."
...
- **document:** {readiness_report_title}
...
- Readiness report published to document
...
- Not publishing report to document
