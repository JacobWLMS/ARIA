...
<action>Invoke the `read-context` task from `{project-root}/_aria/core/tasks/read-context.md` with `context_type: "document_artefact"` and `query: "Architecture"` to search for documents with "Architecture" in the title</action>
...
R/C: Read existing document via the `read-context` task, analyse what sections are complete, jump to next incomplete step. O: List all 8 steps with descriptions, let user choose. X: Confirm with user, then proceed with fresh setup below.
...
<action>Invoke the `read-context` task with `context_type: "document_artefact"` and `scope_id: "product_brief"`</action>
<action>Invoke the `read-context` task with `context_type: "document_artefact"` and `scope_id: "ux_design"` (optional)</action>
...
<action>Call `list_projects` with `team: "{team_name}"` to discover existing ARIA projects</action>

For each Project found:
<action>Call `get_project` with the project ID to load full details</action>
<action>Invoke the `read-context` task with `context_type: "work_items"`, `project: "{project_id}"` to load issues under the project</action>
...
<step n="8" goal="Write architecture to document, link to Projects, and hand off">
...
<action>For each Project, load details, then invoke the platform's project update to update the description with an Architecture link:</action>
...
summary: "Architecture design created and published to document. Ready for epic and story creation."
...
- **document:** {architecture_document_title}
- **Status:** Published
...
**Next Steps:**
1. Review the architecture in documents
