...
Validate that all required artefacts exist on the platform and extract all requirements (FRs, NFRs, and additional requirements from UX/Architecture) needed for epic and story creation.
...
1. **Validate Prerequisites** — load artefacts from the platform and extract requirements (this step)
...
4. **Platform Output** — write projects and issues to the platform with proper linking
</action>
</step>

<step n="1.2" goal="Discover artefacts from key map and documents">
<action>Read `{key_map_file}` and check for the following document entries:</action>
...
<step n="1.3" goal="Load artefacts from the platform">
<action>For each artefact found in the key map, load its content from the platform:</action>

<action>**PRD:** Invoke the `read-context` task from `{project-root}/_aria/core/tasks/read-context.md` with `context_type: "document_artefact"` and `scope_id: "prd"` using the document ID from `documents.prd` in `{key_map_file}`</action>
<action>**Architecture:** Invoke the `read-context` task with `context_type: "document_artefact"` and `scope_id: "architecture"` using the document ID from `documents.architecture` in `{key_map_file}`</action>
<action>**UX Design (if available):** Invoke the `read-context` task with `context_type: "document_artefact"` and `scope_id: "ux_design"` using the document ID from `documents.ux_design` in `{key_map_file}`</action>
<action>**Product Brief (if available):** Invoke the `read-context` task with `context_type: "document_artefact"` and `scope_id: "product_brief"` using the document ID from `documents.product_brief` in `{key_map_file}`</action>

<action>Also check for existing Projects (Epics) in the platform:</action>
<action>Call `list_projects` with `team: "{team_name}"` to discover any existing ARIA projects</action>
...
<action>Ask the user if there are any other documents or documents to include for analysis, and if anything found should be excluded. Wait for user confirmation before proceeding.</action>
</step>

<step n="1.4" goal="Extract Functional Requirements (FRs) from the PRD">
<action>Read the entire PRD content loaded from the platform and extract ALL functional requirements:</action>
...
- All required documents discovered and loaded via key map
