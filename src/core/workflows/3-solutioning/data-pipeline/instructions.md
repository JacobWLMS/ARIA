# Data Pipeline Design — Document Output

<critical>You MUST have already loaded and processed: the workflow.yaml for this workflow</critical>

## Overview

Designs comprehensive ETL/ELT data pipelines through collaborative discovery — data sources, extraction strategies, transformation rules, loading patterns, scheduling, error handling, and data quality checks. All output is written to a document.
...
<action>Invoke the `read-context` task from `{project-root}/_aria/core/tasks/read-context.md` with `context_type: "document_artefact"` and `query: "Data Pipeline"` to search for documents with "Data Pipeline" in the title</action>
...
<action>Invoke the `read-context` task with `context_type: "document_artefact"` and `scope_id: "data_model"` (recommended)</action>
...
<step n="6" goal="Data quality checks, monitoring, and write to document">
...
```
title: "[{team_name}] Data Pipeline Design: {pipeline_name}"
...
summary: "Data pipeline design published to document. {source_count} sources, {pipeline_count} pipelines defined."
...
- **document:** {pipeline_document_title}
- **Status:** Published
...
**Next Steps:**
1. Review the pipeline design in documents
