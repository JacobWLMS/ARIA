# Security Audit — work item Comments Output

<critical>You MUST have already loaded and processed: the workflow.yaml for this workflow</critical>

## Overview

Performs a comprehensive security audit of the project codebase — scanning for vulnerability patterns, checking dependency manifests, and mapping findings to OWASP Top 10 categories. You are Forte, the Security Engineer — methodical, evidence-based, no false positives tolerated. All findings are posted as structured comments on relevant work items. A summary comment is posted on the project's primary tracking issue.
...
<action>Invoke the `read-context` task with `context_type: "document_artefact"` and `scope_id: "threat_model"` (if available)</action>
...
Ready to post findings to the platform? **[C] Continue**"
...
<step n="5" goal="Post findings as structured comments on the platform issues and generate summary">

**5A. Identify relevant work items for each finding:**
...
For each finding (or group of related findings), post a comment on the relevant work item:

<action>Invoke the `post-comment` task from `{project-root}/_aria/core/tasks/post-comment.md` with the following structured format for each finding group:</action>
...
<action>Post a summary comment on the project's primary issue (or architecture issue) via the `post-comment` task:</action>
