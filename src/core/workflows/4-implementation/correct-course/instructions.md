# Correct Course — Platform Integration

<critical>You MUST have already loaded and processed: the workflow.yaml for this workflow</critical>

## Overview

Facilitates mid-cycle course corrections when scope changes, technical blockers, requirement changes, or timeline shifts occur. Assesses impact, proposes platform updates, and executes approved changes.
...
<action>Invoke the `read-context` task from `{project-root}/_aria/core/tasks/read-context.md` with `context_type: "sprint_status"` to load current cycle state</action>
...
<action>For scope/requirement changes: invoke the `read-context` task from `{project-root}/_aria/core/tasks/read-context.md` with `context_type: "work_items"`, `team: "{team_name}"`, `states: ["Backlog", "Todo", "In Progress", "In Review"]` to load active items</action>
...
<step n="4" goal="Propose changes -- define specific platform updates for each affected item">
...
1. **Update issue description/ACs** -- modify the existing issue via the platform's work-item update to reflect changed requirements
2. **Create new remediation issues** -- use the `create-work-item` task to create new issues under the appropriate Project
3. **Re-prioritize issues** -- adjust issue priority within the cycle or backlog
4. **Adjust cycle scope** -- move issues in or out of the current cycle via the platform's work-item update with `cycle`
5. **Document blockers** -- note blocking relationships in issue descriptions (platform does not support formal issue link types, so document dependencies explicitly in descriptions and comments)
...
<step n="5" goal="Execute changes -- apply approved platform updates">
...
<action>For issue updates, invoke the platform's work-item update for each issue with modified descriptions or acceptance criteria</action>
<action>Post a "Course Correction" comment on each affected issue via the `post-comment` task from `{project-root}/_aria/core/tasks/post-comment.md`:</action>
...
<action>For new issues, invoke the `create-work-item` task from `{project-root}/_aria/core/tasks/create-work-item.md` for each new issue:</action>
...
<action>For cycle scope changes, invoke the platform's work-item update to assign or unassign issues from cycles:</action>
...
All approved changes have been applied in the platform. The cycle backlog has been updated to reflect the new priorities and scope.

**Next Steps:**
1. Review updated issues in the platform
