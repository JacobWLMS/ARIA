# Retrospective — Party Mode Multi-Agent Epic Review
...
<critical>
DOCUMENT OUTPUT: Retrospective analysis published to document. Concise insights, lessons learned, action items. User skill level ({user_skill_level}) affects conversation style ONLY, not retrospective content.
...
- All data comes from the platform via the platform MCP tools
- read-context task for project/issue queries and document lookups
- post-comment task for posting comments
- write-document task for creating the retro document
- set-work-item-state task for completing the project
...
<action>Call `list_projects` with `team: "{team_name}"` to load all projects</action>
...
<output>
Tempo (SM): "Here's what I found for the {team_name} team:"
...
<action>Invoke the `read-context` task from `{project-root}/_aria/core/tasks/read-context.md` with `context_type: "work_items"`, `team: "{team_name}"`, `project: "{{project_name}}"` to load all issues</action>
...
<step n="2" goal="Deep Issue Analysis — Extract Data from work items and Comments">
...
<step n="3" goal="Previous Retro Integration — Search documents for Continuity">

<action>Invoke the `read-context` task with `context_type: "document_artefact"` and `query: "Retrospective"` to search for previous retrospective documents</action>

<check if="previous retrospective documents found">
  <action>Load the most recent retrospective document via the `read-context` task</action>
...
<check if="no previous retrospectives found">
  <output>
Tempo (SM): "No previous retrospectives found in documents. This looks like our first one — good time to start the habit."
  </output>
...
<action>Call `list_projects` with `team: "{team_name}"` to find projects that are not completed</action>
...
<check if="next project found">
  ...
  <action>Invoke the `read-context` task with `context_type: "work_items"`, `project: "{{next_project_name}}"` to preview its issues</action>
...
Pitch (QA): "I'd add that if dev records in the platform comments included model changes or deviations from the spec, my test planning would be much more accurate."
...
Riff (Dev): "I'll be honest — when retro action items aren't tracked in the platform, they tend to evaporate. Maybe we should create actual work items for retro commitments."
...
2. Update architecture or technical specifications in documents if applicable
...
Tempo (SM): "Issues marked Done in the platform doesn't always mean truly complete. Let's do a readiness check."
...
<output>
Tempo (SM): "{user_name}, tell me about the testing for '{{project_name}}'. What verification has been done beyond what Pitch flagged in the platform comments?"
</output>
...
Tempo (SM): "I'm going to create actual work items for the key action items so they don't evaporate like last time."
...
<step n="11" goal="Save — Publish to document and Update Issues">
...
```
title: "[{team_name}] Retrospective: {{project_summary}} ({date})"
content: "{compiled_retrospective_content}"
key_map_entry: "documents.retrospective_{{project_name}}"
```

<action>Post a summary comment on a representative issue in the Project via the `post-comment` task:</action>
...
<check if="all issues are Done">
  <action>Mark the project as completed via the platform's project update:</action>
...
summary: "Retrospective for '{{project_name}}' published to document. {{project_status_message}}. {{action_count}} action items assigned."
...
<guideline>Encourage specific examples — reference actual work item identifiers and data from the analysis steps</guideline>
...
<guideline>Deep issue analysis (Step 2) provides rich data for the discussion — mine platform comments thoroughly</guideline>
...
<guideline>All output goes to the platform — never write retrospective content to local files</guideline>
<guideline>Use documents for retrospective reports so future retros can find them</guideline>
