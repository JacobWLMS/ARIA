# Release Plan — document + Milestone Output

<critical>You MUST have already loaded and processed: the workflow.yaml for this workflow</critical>

## Overview

Creates a comprehensive release plan covering version strategy, changelog generation, release checklists, and rollback procedures. All output is written to documents exclusively. When milestones are in use, creates or updates a platform milestone for the release. Posts deployment checklists as comments on affected issues.
...
<step n="1" goal="Initialize and load project context from the platform">

<action>Communicate in {communication_language} with {user_name}</action>

**1A. Check for existing release plans:**

<action>Invoke the `read-context` task from `{project-root}/_aria/core/tasks/read-context.md` with `context_type: "document_artefact"` and `query: "Release Plan"` to search for documents with "Release Plan" in the title</action>
...
<action>Invoke the `read-context` task with `context_type: "document_artefact"` and `scope_id: "architecture"` (if available)</action>
<action>Invoke the `read-context` task with `context_type: "document_artefact"` and `scope_id: "prd"` (if available)</action>

**1C. Load all projects and issues:**

<action>Call `list_projects` with `team: "{team_name}"` to discover all ARIA projects</action>
<action>For each Project, invoke the `read-context` task with `context_type: "work_items"`, `project: "{project_id}"` to load all issues and their statuses</action>
...
- Reference to the platform issue ID
...
- Close milestone in the platform
...
<step n="7" goal="Write release plan to document, create Milestone, and hand off">
...
```
title: "[{team_name}] Release Plan: {version}"
...
**7D. Create or update platform milestone:**
...
<action>For each issue included in the release, invoke the `post-comment` task with the pre-release and release-day checklist items relevant to that issue</action>
...
summary: "Release plan v{version} created and published to document. Milestone created. Ready for release execution."
...
- **document:** {release_plan_document_title}
- **Version:** {version}
...
**Next Steps:**
1. Review the release plan in documents
