...
<action>Load the selected agent personas from `{project-root}/_aria/core/agents/*.agent.yaml`</action>
...
<action>If action items were identified, offer to create work items:</action>

"**{count} action items identified.** Want to create work items for any of these?"
...
- For each selected action item, invoke the `create-work-item` task from `{project-root}/_aria/core/tasks/create-work-item.md` with:
  ```
  team: "{team_name}"
...
<action>Ask the user if they want to save the full summary to a document</action>
<action>If yes: invoke the write-document task at `{project-root}/_aria/core/tasks/write-document.md` with title "{project_name} - Party Mode: {topic summary}"</action>
