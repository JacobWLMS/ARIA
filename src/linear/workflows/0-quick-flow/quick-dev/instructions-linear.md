...
- Call `list_issues` with `team: "{linear_team_name}"`, `labels: ["aria-quick"]`, `states: ["Backlog", "Todo", "In Progress"]`
...
- **Fix Mode** — Issue has `aria-review-failed` label:
  - Call `list_issues` with `parentId: "{selected_issue_id}"`, `labels: ["aria-review-finding"]`, `states: ["Backlog", "Todo", "In Progress"]`
...
Invoke `lock-issue` task with `issue_id: "{selected_issue_id}"`, `action: "lock"`, `agent_name: "dev"`

### 2.2 Set State to In Progress

**New Mode:**
Invoke `set-issue-state` task:
...
- Invoke `read-linear-context` task with `context_type: "document"` and `scope_key: "architecture"` to fetch the Architecture document (if exists)
...
6. Set the finding sub-issue state to Done via `set-issue-state`
...
Call `save_comment` on the issue:
...
Invoke `lock-issue` task with `issue_id: "{selected_issue_id}"`, `action: "unlock"`, `agent_name: "dev"`
...
### Success Criteria
- Dev Agent Record posted as Linear comment with all sections
...
- Git operations blocking Linear completion (git failures should never block)
