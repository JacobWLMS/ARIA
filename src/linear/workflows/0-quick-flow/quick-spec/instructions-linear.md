# Quick Spec — One-Off Tech Spec to Linear Issue
...
{if detected_as_bug OR user says "bug"}
  ...
  - **[Q] Quick Bug** — Create a minimal Linear issue with repro steps and jump straight to implementation (skip spec). Best for clear, isolated bugs.
...
<action>Lock the issue via `lock-issue` task, then invoke the dev-story workflow at `{project-root}/_aria/linear/workflows/4-implementation/dev-story/workflow-linear.yaml` to begin implementation. Skip the spec phase entirely.</action>
...
"**Quick Spec Mode** — Creating a one-off tech spec as a Linear issue. No PRD, no Project, no cycle. Let's scope this and ship it."

### 1.2 Check for Existing Quick-Flow Issues

Search for existing quick-flow issues that might overlap:
- Call `list_issues` with `team: "{linear_team_name}"`, `labels: ["aria-quick"]`, `states: ["Backlog", "Todo", "In Progress", "In Review"]`
...
**Check Linear for architectural context:**
- Call `list_documents` to search for architecture or project context documents
...
- Project context and Linear architecture docs loaded if available
...
**Progress: Step 4 of 5** — Next: Create Linear Issue
...
"**Select:** [C] Create Linear Issue [E] Edit [Q] Questions [A] Advanced Elicitation [P] Party Mode [R] Adversarial Review"
...
- Call `list_users` to find the user matching `{default_assignee}` by name or email
- Record the user ID for assignment

### 5.2 Create the Linear Issue
...
**Notes:**
- The `labels` parameter accepts label names — the Linear MCP server resolves them to IDs
- If a label does not yet exist, call `create_issue_label` with `team: "{linear_team_name}"`, `name: "{label_name}"` first

### 5.3 Link Dependencies

If dependencies were identified that reference existing Linear issues:
- Embed issue identifiers directly in the description under the Dependencies section
- Optionally use `save_issue` with `blocks` or `blockedBy` params to create formal dependency links
...
2. Or assign it manually in Linear for later implementation
...
### Success Criteria
- Linear issue created with complete tech spec as description
