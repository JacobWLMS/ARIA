...
<action>Search for existing project context on Linear:</action>
<action>Call `list_documents` and search for documents with "Project Context" in the title</action>

If found:
- Load via `get_document`
...
<step n="3" goal="Compile, optimize, save locally, publish to Linear Document">
...
### Publish to Linear Document

<action>Invoke the `write-to-linear-doc` task with:</action>

```
title: "[{linear_team_name}] Project Context: {project_name}"
...
- Saved both locally and on Linear
