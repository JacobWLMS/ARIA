# Research — Linear Document Output

<critical>You MUST have already loaded and processed: the workflow-linear.yaml for this workflow</critical>

## Overview

Conducts comprehensive research (market, domain, or technical) using web search and verified sources. The research type is determined by user input. Output is written to a Linear Document as a research report.
...
<action>Invoke the `read-linear-context` task with `context_type: "project_overview"` to check for existing context on Linear</action>
...
<step n="6" goal="Synthesize findings, write to Linear Document, and hand off">
...
```
title: "[{linear_team_name}] Research: {research_topic}"
...
summary: "{research_type} research on '{research_topic}' published to Linear Document."
