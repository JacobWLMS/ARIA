...
### 1.1 Locate the PRD on Linear

Invoke the `read-linear-context` task from `{project-root}/_aria/linear/tasks/read-linear-context.md` with `context_type: "document"` and `scope_key: "prd"` to locate the PRD document.

If the PRD is not found via key map, search Linear directly:
...
- "I cannot find an existing PRD on Linear. Please check or create one first with Create PRD."
- EXIT workflow.

### 1.2 Load Full PRD Content

Call `get_document` with the selected document ID to load the full PRD content. Store the `document_id`, `title`, and content for later update.
...
"**Edit Mode: Updating existing PRD on Linear.**
...
### Success Criteria
- PRD located and fully loaded from Linear
- Comments retrieved
...
<step n="4" goal="Update the Linear Document with the edited PRD">
...
**Next Steps:**
1. Review the updated PRD on Linear
