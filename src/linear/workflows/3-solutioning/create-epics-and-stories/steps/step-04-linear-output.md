# Step 4: Output Projects (Epics) and Issues (Stories) to Linear

**This step replaces the default file-write output step.** The elicitation and analysis steps (1-3) remain unchanged — they build the epics/stories structure in the agent's working memory. This step writes that structure to Linear.
...
- Configuration loaded: `{linear_team_name}`, `{agent_label_prefix}`, `{key_map_file}`, `{project_prefix}`
- PRD Linear Document exists (look up `documents.prd` in `{key_map_file}`)
...
<action>If `default_assignee` is non-empty, call `list_users` with `query: "{default_assignee}"` to resolve the Linear user</action>
...
<action>Call `list_issue_labels` with `team: "{linear_team_name}"` to get existing labels</action>
...
<action>For each missing label, call `create_issue_label` with:</action>
...
<action>Call `list_teams` with `query: "{linear_team_name}"` to resolve the team ID</action>
<action>Record the team ID for use in project and issue creation</action>
<action>If team not found, warn user and attempt to use `{linear_team_id}` from module.yaml as fallback</action>
</step>

<step n="4.3" goal="Create Linear Projects (Epics)">
<action>For each Epic in the structure, invoke the `create-project-epic` task from `{project-root}/_aria/linear/tasks/create-project-epic.md` with:</action>
...
<step n="4.4" goal="Create Linear Issues (Stories) under Projects">
<action>For each Story under each Epic, call `save_issue` with `project` set to the Epic's project ID:</action>
...
<action>The `project` field links the Issue to its Project (Epic) automatically — issues appear under the project in Linear's project view</action>
...
<action>For stories that have dependencies on other stories, create `blocks`/`blockedBy` relationships:</action>
...
```
id: "{dependent_story_issue_id}"
blockedBy: ["{blocking_story_issue_id}"]
```
...
```
id: "{blocking_story_issue_id}"
blocks: ["{dependent_story_issue_id}"]
```

<action>This creates bidirectional dependency links visible in Linear's issue view</action>
</step>
...
<action>For each Project (Epic) created, call `save_project` to append a Related Documents section to the description:</action>
...
<action>This ensures downstream workflows can look up Linear project and issue IDs</action>
</step>
...
summary: "Created {epic_count} projects and {story_count} issues in Linear. Ready for architecture review or implementation readiness check."
...
**Linear Output Complete**

| Type | Count | Team |
|---|---|---|
| Projects (Epics) created | {epic_count} | {linear_team_name} |
| Issues (Stories) created | {story_count} | {linear_team_name} |
...
**Next Steps:**
1. Review the created projects and issues in Linear
...
- All epics created as Linear Projects with full descriptions and FR coverage
- All stories created as Linear Issues with complete acceptance criteria in descriptions
