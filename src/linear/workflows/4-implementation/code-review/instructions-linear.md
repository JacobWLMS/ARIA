...
<action>Call `list_issues` with `team: "{linear_team_name}"`, `states: ["In Review"]`, and `limit: 5`</action>
...
<action>Invoke `lock-issue` task from `{project-root}/_aria/linear/tasks/lock-issue.md` with `issue_id: "{selected_issue_id}"`, `action: "lock"`, `agent_name: "code-review"`</action>
...
<step n="3" goal="Create Linear sub-issues for findings">

**Progress: Step 3 of 6** -- Next: Post Summary

<action>For each finding with severity Critical or Major, call `save_issue`:</action>
...
For each sub_issue created in Step 3:
  Invoke `set-issue-state` task with:
    issue_id: "{sub_issue_id}"
    target_state: "Done"
    comment: "Non-blocking finding -- story passed review."
```

<action>Invoke `set-issue-state` task from `{project-root}/_aria/linear/tasks/set-issue-state.md`:</action>
...
<action>Call `save_issue` to add review labels:</action>
...
<action>Call `list_issues` with `project: "{parent_project_name}"`, `states: ["Backlog", "Todo", "In Progress", "In Review"]`</action>
...
<action>Invoke `lock-issue` task from `{project-root}/_aria/linear/tasks/lock-issue.md` with `issue_id: "{selected_issue_id}"`, `action: "unlock"`, `agent_name: "code-review"`</action>
