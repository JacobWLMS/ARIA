# Create Story — Linear Output

<critical>You MUST have already loaded and processed: the workflow-linear.yaml for this workflow</critical>

## Overview

Prepares a story with full development context. The analysis steps follow the create-story checklist. The output step writes to Linear (updating the issue description and adding dev notes as a comment) and sets the issue state to "Todo" (ready for dev).
...
<action>Invoke `read-linear-context` task from `{project-root}/_aria/linear/tasks/read-linear-context.md` with `context_type: "sprint_status"` to find issues in Backlog status</action>
<action>Call `list_issues` with `team: "{linear_team_name}"`, `states: ["Backlog"]`, and `limit: 5` to find unprepared backlog issues</action>
...
<step n="2" goal="Gather context from Linear">

**Progress: Step 2 of 7** -- Next: Analyse and Prepare

### 2.1 Story Detail Context

<action>Invoke `read-linear-context` task with `context_type: "story_detail"` and `scope_key: "{selected_issue_id}"` to fetch:</action>
- Full issue description and acceptance criteria
- Parent Project details
- Architecture document from Linear Documents (if exists)

### 2.2 Previous Story Learnings

<action>Invoke `read-linear-context` task with `context_type: "previous_story_learnings"` to fetch:</action>
...
**Progress: Step 3 of 7** -- Next: Write to Linear
...
5. Check for dependencies on other issues — if dependencies exist, set `blockedBy` relationships via `save_issue` in step 4 (Linear supports `blocks`/`blockedBy` natively)
...
**Progress: Step 3.5 of 7** -- Next: Write to Linear
...
<step n="4" goal="Write enriched story to Linear">

**Progress: Step 4 of 7** -- Next: Create Sub-issues

<action>Call `save_issue` to update the issue description with the enriched content and estimate:</action>
...
<action>Call `save_comment` to post the dev notes separately (keeps them visible in the comment stream):</action>
...
<action>This makes dependencies visible in Linear's issue view and prevents the orchestrator from dispatching blocked stories</action>
...
<step n="5" goal="Create sub-issues in Linear (if tasks were decomposed)">
...
<action>Invoke `set-issue-state` task from `{project-root}/_aria/linear/tasks/set-issue-state.md` with:</action>
...
<action>Invoke `post-handoff` task from `{project-root}/_aria/linear/tasks/post-handoff.md` with:</action>
...
- **Linear Issue:** {selected_issue_identifier}
- **Status:** Todo
