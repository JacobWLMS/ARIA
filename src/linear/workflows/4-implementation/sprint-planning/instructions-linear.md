# Sprint Planning — Linear Cycle Management

<critical>You MUST have already loaded and processed: the workflow-linear.yaml for this workflow</critical>

## Overview

This workflow manages sprint planning via Linear Cycles. It discovers stories from Linear, helps prioritize them for a cycle, and assigns them. Linear Cycles are created in the Linear UI -- this workflow lists available cycles and assigns issues to them.

---

<workflow>

<step n="1" goal="Read current Linear project state">
...
<action>Call `list_cycles` with `team: "{linear_team_name}"` to find all cycles (active, upcoming, completed)</action>
...
<action>If an active cycle exists, call `list_issues` with `team: "{linear_team_name}"`, `cycles: ["{active_cycle_name}"]` to get issues in the cycle</action>

### 1.3 Load Backlog Issues

<action>Call `list_issues` with `team: "{linear_team_name}"`, `states: ["Backlog", "Todo"]` to find unplanned backlog issues not assigned to any cycle</action>

### 1.4 Load Projects (Epics)

<action>Call `list_projects` with `team: "{linear_team_name}"` to get all projects for epic overview</action>

### 1.5 Previous Story Learnings

<action>Invoke the `read-linear-context` task from `{project-root}/_aria/linear/tasks/read-linear-context.md` with `context_type: "previous_story_learnings"` to incorporate insights from recently completed stories</action>

### 1.6 Sprint Velocity History

<action>Call `list_documents` with `query: "Retrospective"` to find previous retrospective documents</action>
<action>If retrospective documents exist, call `get_document` for the most recent 1-3 to extract velocity data (story points completed per cycle)</action>
...
<action>Call `list_issues` with `team: "{linear_team_name}"`, `states: ["Done"]`, `limit: 20` to count completed story points</action>

<action>Store the velocity data for capacity planning in step 4</action>

### 1.7 Build Inventory

<action>Build a complete inventory of all projects, issues, and their current states from Linear</action>
...
<action>Present available upcoming cycles from the `list_cycles` results:</action>
...
<action>
  {if autonomy_level == "interactive"}
    Ask user to select a cycle, or ask them to create one in the Linear UI.
  {else}
...
```
No upcoming cycles found. Please create a cycle in the Linear UI:
...
<action>If any backlog issues lack estimates, invoke the `refine-backlog` task from `{project-root}/_aria/linear/tasks/refine-backlog.md` with `team: "{linear_team_name}"`</action>
...
<action>For each selected issue, call `save_issue` to assign it to the cycle:</action>
...
<action>For each selected issue, call `save_comment` with a sprint planning note:</action>
...
**Linear Board:** View in Linear at your team's Cycles view
...
3. Re-run Sprint Planning to refresh status from Linear
...
## Status Flow Reference (Linear-mapped)

```
Project: Not Started -> Started -> Completed
         (Managed by Linear based on issue progress)

Issue:   Backlog -> Todo -> In Progress -> In Review -> Done
         (Set via save_issue with state: "Status Name")
```

All state changes use `save_issue` with `state: "Status Name"` -- no transition IDs needed in Linear.

## Linear Cycle Notes

| Operation | Linear MCP Tool |
|---|---|
| List cycles | `list_cycles` with team filter |
| Create cycle | Not available via MCP -- create in Linear UI |
| Assign issue to cycle | `save_issue` with `cycle: "Cycle Name"` |
| List issues in cycle | `list_issues` with `cycles` filter |
