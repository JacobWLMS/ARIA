# Sprint Status — Linear Cycle Dashboard

<critical>You MUST have already loaded and processed: the workflow-linear.yaml for this workflow</critical>

## Overview

Provides a read-only cycle and project progress dashboard by polling Linear for current cycle state, issue statuses, and velocity metrics. Optionally publishes the status as a Linear Document. No state changes or modifications are made.

---

<workflow>

<step n="1" goal="Poll Linear state -- load cycle, issues, and project data">
...
<action>Call `list_cycles` with `team: "{linear_team_name}"` to identify the current active cycle</action>

### 1.2 Load Cycle Issues

<action>If an active cycle exists, call `list_issues` with `team: "{linear_team_name}"`, `cycles: ["{active_cycle_name}"]` to load all issues in the cycle</action>
...
<action>For each Project, call `list_issues` with `project: "{project_name}"` to get status counts per Project</action>
...
<step n="3" goal="Present status and optionally publish to Linear Document">
...
<action>Ask the user if they would like to publish this status as a Linear Document</action>

<action>If the user wants to publish, invoke the `write-to-linear-doc` task from `{project-root}/_aria/linear/tasks/write-to-linear-doc.md` with:</action>
...
<action>For each Project with issues in the active cycle, call `save_comment` on a representative issue:</action>
